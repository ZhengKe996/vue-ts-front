<template>
  <m-popover class="flex items-center" placement="bottom-left">
    <template #reference>
      <div class="guide-my relative flex items-center p-0.5 rounded-sm cursor-pointer duration-200 outline-none hover:bg-zinc-100 dark:hover:bg-zinc-900" v-if="userStore.getToken">
        <img v-lazy class="w-3 h-3 rounded-sm" :src="UserImg" />
        <m-svg-icon class="h-1.5 w-1.5 ml-0.5" fillClass="fill-zinc-900 dark:fill-zinc-300" name="down-arrow"> </m-svg-icon>
    ...
    <div v-if="userStore.getToken" class="w-[140px] overflow-hidden">
      <div v-for="item of menuArr" :key="item.id" class="flex items-center p-1 cursor-pointer rounded hover:bg-zinc-100/60 dark:hover:bg-zinc-800" @click="onItemClick(item.path)">
